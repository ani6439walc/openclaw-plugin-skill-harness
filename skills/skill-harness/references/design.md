...
2. Concrete triggers and examples.
3. Optional durable short `keywords`.
4. One durable plain-text body guidance sentence and direct skills worth listing in frontmatter.
5. Neighboring or colliding intents.
...
- Check the proposed boundary and neighboring intents against the cluster consistency criteria in `references/clustering.md`.
...
- Required frontmatter: `triggers[]` and `examples[]`; the complete body is one `guidance` sentence.
...
- The chosen boundary passes the cluster consistency criteria from `references/clustering.md`.
