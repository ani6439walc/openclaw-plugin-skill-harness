...
6. Hybrid trigger/example QMD uses forced expansion and reranking. Its top result over `0.85` routes directly. Scores from `0.65` through `0.85` give the classifier a small candidate set; scores from `0.35` through under `0.65` give it a larger set. Empty, stale, failed, or under-`0.35` retrieval runs one full-catalog classifier call.
