...
For `skill-candidate`, `process-gap`, `successful-pattern`, and `behavior-fix`, Review may instead create one new `experiences/<skill>/<entry-id>.md`. The skill must have been successfully observed in the current turn and still be visible to that agent. The reviewer stages one file and declares one `skill-experience` finding; the host validates the strict experience schema, identity, visibility, non-overwrite rule, and concurrent creation before applying it. Existing experience records cannot be refined or deleted by Review.
...
The reviewer does not write source files, bundled skills, OpenClaw config, memory files, arbitrary filesystem paths, or existing experience records.
...
Only `applied` and `nofinding` complete a skill inventory epoch. The runtime Review event, including any host-validated skill-experience creation, and completed epoch are written atomically to schema-v7 `review.json`; keyword-triggered audits are retained separately in `historicalKeywordAudits`. Reviewer errors, parse or schema rejection, validation failure, queue failure, and review-log write failure clear the in-memory reservation without completing the epoch, so a later accepted turn can retry. An unreadable or incompatible review log disables placement selection while preserving ordinary Review triggers.

Schema v7 is current-only: schema-v6 and older logs, legacy string events, legacy outcomes, aliases, and unknown fields are rejected rather than migrated. Delete or archive an incompatible runtime `review.json` before enabling this version.
