...
Schema-v5 `stats.json` retains intent, skill, tool, routing, projection, inventory, and daily telemetry, and adds per-intent `routeReasons` for `qmd-keyword`, `qmd-hybrid`, and `llm-classifier` with selected-route score aggregates. QMD scores use retrieval hit scores; classifier scores use classifier confidence. The runtime-health report exposes aggregate inventory counts without agent or skill names.
...
- Dynamic context is prepended with blank line separation after the turn timestamp using a single-line advisory header (`Inferred intent and candidate skills (advisory, non-user input; load with \`skill_view\` if relevant):`when candidate skills are present, or`Inferred user intent from conversation (advisory, non-user input):`when intent-only) preceding`<skill_harness_plugin>`. Inside `<skill_harness_plugin>`, the selected intent merges name and guidance as `<intent name="${intent}">\n ${guidance}\n</intent>`, candidate skills render as `<skill name="${name}">\n ${description}\n    ${experiences}\n</skill>`, and `<context_policy>`is omitted. The renderer emits neither`<<<BEGIN_SKILL_HARNESS_CONTEXT>>>` nor OpenClaw reserved delimiters; sanitization markers are input boundaries only.
- Static configured skills context uses markdown header `### Configured skills` with guidance `When relevant, load with \`skill_view\` before proceeding:`, wrapping skills as `<configured_skills>\n <skill name="${name}">\n ${description}\n </skill>\n</configured_skills>`. `<path>`elements are not emitted in prompt context; agents query paths and file contents on demand via`skill_list`and`skill_view`.
