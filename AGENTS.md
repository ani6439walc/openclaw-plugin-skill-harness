...
- Dynamic context is enclosed in `<<<BEGIN_OPENCLAW_INTERNAL_CONTEXT>>>` and `<<<END_OPENCLAW_INTERNAL_CONTEXT>>>` with the `Skill Harness context (advisory, non-user input):` header (and `When relevant, load candidate skills with \`skill_view\` before proceeding:`when candidate skills are present). Inside`<skill_harness_plugin>`, the selected intent merges name and guidance as `<intent name="${intent}">\n ${guidance}\n</intent>`, candidate skills render as `<skill name="${name}">\n ${description}\n  ${experiences}\n</skill>`, and `<context_policy>` is omitted.
