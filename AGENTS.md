...
The dynamic pipeline runs intent matching and input skill discovery in parallel. Intent matching has three stages: Step 1 (QMD Keyword BM25 over intent `keywords`), Step 2 (QMD Hybrid Example/Keyword Search with conversation expansion), and Step 3 (one constrained unified selection call only when the visibility-filtered union of intent and/or skill candidates is non-empty). A direct intent with no skill candidates does not call a model; no qualifying intent may produce a valid skills-only context; `maxInjectedSkills: 0` short-circuits skill discovery; model, parsing, or validation failure injects no heuristic fallback skills. Direct skill evidence combines bounded typo-aware name matching (strip URLs before tokenization) and managed `SkillQmdIndex` retrieval over meta/body/references. The final renderer emits one optional `<intent>` and one optional `<matched_skills>` block only. Topic checkers (`previousTopic`), runtime task complexity scoring (`complexity`), classifier `suggestion` fields, session curation (`curation` / `curationAppliedCount`), instruction writers, legacy `fastpath` / `candidate` frontmatter fields, and legacy pseudo-headers (`[Skill Harness Context...]`, `[User Message]:`) are obsolete and strictly forbidden. Do not use OpenClaw core's reserved `<<<BEGIN_OPENCLAW_INTERNAL_CONTEXT>>>` delimiters in plugin prompt output as core runtime logic strips them from visible turns; these delimiters are sanitizer-only input boundaries. Projection reasons use `exact-keyword-overlap`. Intent YAML frontmatter only permits `triggers`, `examples`, `keywords`, and `skills` formatted in canonical key order (`triggers`, `examples`, `keywords`, `skills`) with skill names strictly lowercase; `triggers[]` and `examples[]` are required while `keywords[]` and `skills[]` are optional; the body is strictly single-line plain-text routing `guidance`.
