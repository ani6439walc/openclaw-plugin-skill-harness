...
- Plugin-owned `workingSetSkills` is the only static working-set source: `agents.<id>` precedes `defaults`, then workspace-only skills append. Native OpenClaw `agents.*.skills` is not read or mutated by this plugin. Static prompt injection uses `### Working set skills` / `<working_set_skills>` and never renders paths.
