...
- General/knowledge-base style lessons are recordable only when they are reusable workflows or decision steps, costly error recovery paths, critical parameters/settings/prerequisites, stable user preference or style rules, multi-attempt successful solutions with failure reasons and success conditions, reusable templates/checklists/formats, or stable external dependency/resource locations.
- General/knowledge-base style lessons are not recordable when they are one-off Q&A, pure conceptual explanations without concrete steps or decision criteria, or conclusions without specific reusable context.
- Experience/non-auto-skill lessons are recordable only when they capture a skill-specific pitfall and fix, error message or localization path, result-shaping parameter/configuration, reusable prompt/template/workflow, dependency or asset path, project entry point/module location, or required step ordering.
- Experience/non-auto-skill lessons are not recordable when they are pure theory, conclusions without reproducible steps, or one-time non-reusable operations.
- When evidence resembles a knowledge-base or experience entry, distill only the reusable title, context, solution steps, key paths, parameters, and keywords that directly improve the matched intent's Guidelines, Response Strategy, Concrete Workflow, or Experience; do not propose external file formats or writes.
