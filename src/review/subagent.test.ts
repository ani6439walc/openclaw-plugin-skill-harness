...
const REQUIRED_WEAK_INTENT_REVIEW_PROMPT_SNIPPETS = [
  ...
  "Frontmatter is classification-only and contains triggers[], examples[], one required domain, optional fastpath metadata, and optional skills[]",
...
describe("parseReviewFindings", () => {
  ...
  it("accepts no-finding reason codes without persisting them as review findings", () => {
    const parsed = parseReviewFindings(
      JSON.stringify({
        findings: [
          {
...
\`\`\``;
  ...
  it("parses trigger keyword findings for direct review log updates", () => {
    const parsed = parseReviewFindings(
      JSON.stringify({
        findings: [
          {
  ...
  it("fails open without retrying the fallback review model after parse failure", async () => {
    ...
    await expect(
      runReviewSubagent({
        api,
        config: resolveConfig({
          review: { enabled: true, modelFallback: "google/fallback-review" },
        }),
        agentId: "main",
        intentDirectory,
        modelRef: { provider: "google", model: "review" },
  ...
  it("returns subagent-error without invoking the configured fallback", async () => {
    ...
    await expect(
      runReviewSubagent({
        api,
        config: resolveConfig({
          review: { enabled: true, modelFallback: "google/fallback" },
        }),
        agentId: "main",
        intentDirectory: createIntentDirectory(),
        modelRef: { provider: "bifrost", model: "glm-5" },
