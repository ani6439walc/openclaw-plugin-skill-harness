...
export function isEnabledForAgent(
  ...
  return config.routing.scope.agents.includes(agentId);
}
...
export function isAllowedChatType(
  ...
  return config.routing.scope.chatTypes.includes(chatType);
}
...
export function isAllowedChatId(
  ...
  if (
    config.routing.scope.allowedChatIds.length > 0 &&
    !config.routing.scope.allowedChatIds.includes(chatId)
  )
    return false;
  if (config.routing.scope.deniedChatIds.includes(chatId)) return false;
  return true;
}
