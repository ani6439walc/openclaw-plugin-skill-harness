...
describe("intention-hint backlog mode", () => {
  it("routes explicit backlog requests to the transactional reference", () => {
    ...
    expect(reference).toContain('intention_hint_evolution({ action: "show" })');
    ...
    expect(reference).toContain('intention_hint_evolution({ action: "list" })');
    expect(reference).not.toContain("/intention-hint evolution");
